...
# This v2 file lives next to the original generator in .github/workflows.
ORIGINAL = Path(__file__).with_name("KnowledgeCombo_generate_daily.py")
        ...
        for difficulty in orig.DIFFICULTIES:
            ...
            for broaden in (False, True):
                ...
                more = orig.select_unused_combos(
                    ...
                    candidate_budget,
                    target_date,
                    f"{topic}:{difficulty}:{'broad' if broaden else 'exact'}:text-dedup-v2",
                ...
                for combo in more:
                    ...
                    # Do not fail on an exact historical text duplicate.
                    # Skip it and keep searching for another valid unused candidate.
            ...
            for idx, (combo, question) in enumerate(zip(chosen, chosen_questions), 1):
                ...
                rng = random.Random(
                    int(orig.stable_hash(target_date, topic, difficulty, str(idx), "v2")[:16], 16)
                )
